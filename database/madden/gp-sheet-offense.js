/* gp-sheet-offense.js: THE OFFENSIVE CALL SHEET, lifted from Bears Playbook Madden 27.html by
   Madden27 Engine Reference/extract-gp-offense.js. DO NOT EDIT: edit the
   playbook and re-run the extractor. Lines 6268-6967 of the playbook, with the
   outside names it reaches for supplied through GPX (the crest is GPX.logo). */
...
function gpLoad(){
  try{
    ...
    if(raw){
      ...
      /* The X-Factor is stored by NAME, so a sheet saved before a roster change
         keeps showing a guy who is no longer on the team (DJ Moore). Drop any
         pick whose player is off the current roster and fall back to the
         recommended one - the sheet heals itself instead of needing a reset. */
      if(s.xFactor && s.xFactor.player){
        const roster = ((typeof PERSONNEL4 !== 'undefined' && PERSONNEL4.players) || [])
          .map(p => p.name);
        if(roster.length && roster.indexOf(s.xFactor.player) === -1)
          s.xFactor = Object.assign({}, GP.recommended.xFactor);
      }
      return s;
    }
  }catch(e){}
...
function gpViewHTML(){
  ...
  return `
    ...
    <div class="gp-sheet">
      <div class="gp-ph">
        <img class="gp-ph-o" src="${GPX.logo}" alt="">
        ${GP.headerArt
...
function wireGp7(){
  ...
  function switchMode(to){
    if(rm){ to === 'forms' ? showFormations() : render(); return; }
    root.classList.add('gp-fade');
    setTimeout(()=>{
      to === 'forms' ? showFormations() : render();
      root.classList.add('gp-fade');
  ...
  function wireForm(){
    ...
    root.querySelectorAll('.gp-knob').forEach(b=>b.addEventListener('click', ()=>{
      if(b.dataset.tab === 'plan') switchMode('plan');
    }));
    wireFormCalls();
  }
  ...
  function wirePlan(){
    ...
    root.querySelectorAll('.gp-knob').forEach(b=>b.addEventListener('click', ()=>{
      if(b.dataset.tab === 'forms') switchMode('forms');
    }));
  }
